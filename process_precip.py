...
def process_precip(matfile, outcsv):
    ...
    hist = pd.DataFrame(mat['pr_historical_yrly'], columns=hist_names, index=hist_years)
    rcp45 = pd.DataFrame(mat['pr_rcp45_yrly'], columns=rcp45_names, index=future_years)
    rcp85 = pd.DataFrame(mat['pr_rcp85_yrly'], columns=rcp85_names, index=future_years)
